...
function editar(rowId) {
	bloquearPantalla();
	$.ajax({
		url : "modificarIdentidadManual.htm",
		type : "GET",
		async : false,
		cache : false
	}).done(
...
function updateProcesar() {
	var selectedRowIds = $("#grilla_validacionManual")
			.getGridParam("selarrrow");
	if (selectedRowIds.length > 0) {

		var accionPreEjecucion = "confirmarUpdateProcesar('" + selectedRowIds + "')";
		var accionPostEjecucion = "actualizarLista()";

		mensajeConfirmacionNuevo(accionPreEjecucion, accionPostEjecucion);
	} else {
		mostrarMensaje(VG_TIPO_MENSAJE_WARNING,
				"primero selecciona algunas filas");
	}
}

function confirmarUpdateProcesar(selectedRowIds) {
	var params = "";
...
function updateError() {
	...
	if (selectedRowIds != undefined && selectedRowIds.length > 0) {
		if (confirm("Los registros seleccionados serán reportados con Error ¿Desea Continuar?")) {
			...
			$
					.ajax({
						url : 'errorValidacionManual.json',
						data : 'params=' + params,
						type : 'POST',
						async : false,
						cache : false,
